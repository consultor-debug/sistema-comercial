...
interface ReportData {
    inventory: { status: string; count: number }[];
    totalQuotations: number;
    projects: { id: string; name: string; _count: { lots: number; quotations: number } }[];
    recentActivity: { id: string; clienteNombres?: string; clienteApellidos?: string; precioFinal: number; createdAt: Date | string; user: { name: string | null } | null; lot: { manzana: string; loteNumero: string | number } }[];
}
...
export default function ReportsPage() {
    ...
    const totalLots = data?.inventory.reduce((acc: number, item) => acc + item.count, 0) || 0
    const salesCount = data?.inventory.find((i) => i.status === 'VENDIDO')?.count || 0
