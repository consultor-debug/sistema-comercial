...
export const LotMarker: React.FC<LotMarkerProps> = ({
    ...
    const baseRadius = Math.max(10, imageSize.width * 0.014)

    /* ── CIRCLE ─────────────────────────────────────────────── */
    if (lot.mapShapeType === 'circle' && shapeData.x !== undefined && shapeData.y !== undefined) {
        const r  = shapeData.radius ? Math.max(shapeData.radius, baseRadius) : baseRadius
        ...
        return (
            ...
            >
                ...
                <circle cx={cx} cy={cy} r={r}
                    fill={fill}
                    fillOpacity={isSelected ? 0.82 : fillOpacity}
    ...
    if (lot.mapShapeType === 'polygon' && shapeData.points) {
        ...
        return (
            ...
            >
                ...
                <polygon points={pointsStr}
                    fill={fill}
                    fillOpacity={isSelected ? 0.78 : fillOpacity}
                    stroke={isSelected ? '#fff' : stroke}
