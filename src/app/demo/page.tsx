...
export default function DemoPage() {
    ...
    const [selectedLot, setSelectedLot] = React.useState<any | null>(null)
    const [isModalOpen, setIsModalOpen] = React.useState(false)
    const [isMounted, setIsMounted] = React.useState(false)

    React.useEffect(() => {
        setLots(generateDemoLots())
        setIsMounted(true)
    }, [])

    const handleLotClick = (lot: any) => { // eslint-disable-line @typescript-eslint/no-explicit-any
        setSelectedLot(lot)
        setIsModalOpen(true)
    }
